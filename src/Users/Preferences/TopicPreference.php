...
final class TopicPreference implements BaseModel
{
    ...
    public static function with(
        ...
        $obj->defaultStatus = $defaultStatus instanceof PreferenceStatus ? $defaultStatus->value : $defaultStatus;
        $obj->status = $status instanceof PreferenceStatus ? $status->value : $status;
        $obj->topicID = $topicID;
        $obj->topicName = $topicName;

        null !== $customRouting && $obj->customRouting = array_map(fn ($v) => $v instanceof ChannelClassification ? $v->value : $v, $customRouting);
    ...
    {
        $obj = clone $this;
        $obj->customRouting = array_map(fn ($v) => $v instanceof ChannelClassification ? $v->value : $v, $customRouting);
